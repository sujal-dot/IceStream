"""
IceStream Day 13 — Writer A Client for Iceberg ACID Audit
Appends controlled records with prefix 'acid_a_' to bronze.checkout_events via PyIceberg.
"""
import sys
import uuid
import time
...
def generate_writer_a_batch(count: int = 500) -> tuple[List[Dict[str, Any]], List[str]]:
    """
    Generates a batch of realistic checkout events for Writer A.
    All event_ids start with 'acid_a_' followed by a UUID.
    """
    events = []
    event_ids = []
    ...
    for i in range(count):
        eid = f"acid_a_{uuid.uuid4().hex[:12]}"
        event_ids.append(eid)
        event = {
            "event_id": eid,
            "event_time": now,
            "customer_id": f"cust_a_{i:04d}",
            "session_id": f"sess_a_{i:04d}",
            "order_id": f"ord_a_{i:04d}",
            "product_id": f"prod_{100 + (i % 20)}",
            "amount": Decimal(f"{(19.99 + (i * 0.5)):.2f}"),
            "currency": "USD",
...
class WriterA:
    """
    Writer A worker executing controlled Iceberg append operations with optimistic concurrency retry handling.
    """
    ...
    def run(self) -> Dict[str, Any]:
        """
        Connects to Iceberg, appends Writer A records with optimistic retries, and returns detailed audit metrics.
        """
        ...
        try:
            ...
            events, event_ids = generate_writer_a_batch(self.record_count)
...
if __name__ == "__main__":
    writer = WriterA(record_count=100)
    audit = writer.run()
    print("Writer A Result:", audit)
