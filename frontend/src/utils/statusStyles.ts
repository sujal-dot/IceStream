import { NodeStatus } from '../types/lineage';

export interface StatusStyle {
  label: string;
  badgeBg: string;
  badgeText: string;
  borderColor: string;
  dotBg: string;
  glowColor: string;
  ariaLabel: string;
}

export const STATUS_STYLES: Record<string, StatusStyle> = {
  HEALTHY: {
    label: 'Healthy',
    badgeBg: '#10B9811A',
    badgeText: '#059669',
    borderColor: '#10B981',
    dotBg: '#10B981',
    glowColor: 'rgba(16, 185, 129, 0.25)',
    ariaLabel: 'Status: Healthy',
  },
  WARNING: {
    label: 'Warning',
    badgeBg: '#F59E0B1A',
    badgeText: '#D97706',
    borderColor: '#F59E0B',
    dotBg: '#F59E0B',
    glowColor: 'rgba(245, 158, 11, 0.25)',
    ariaLabel: 'Status: Warning',
  },
  DEGRADED: {
    label: 'Degraded',
    badgeBg: '#F59E0B1A',
    badgeText: '#D97706',
    borderColor: '#F59E0B',
    dotBg: '#F59E0B',
    glowColor: 'rgba(245, 158, 11, 0.25)',
    ariaLabel: 'Status: Degraded',
  },
  CRITICAL: {
    label: 'Critical',
    badgeBg: '#EF44441A',
    badgeText: '#DC2626',
    borderColor: '#EF4444',
    dotBg: '#EF4444',
    glowColor: 'rgba(239, 68, 68, 0.25)',
    ariaLabel: 'Status: Critical',
  },
  CIRCUIT_OPEN: {
    label: 'Circuit Open',
    badgeBg: '#EF44441A',
    badgeText: '#DC2626',
    borderColor: '#EF4444',
    dotBg: '#EF4444',
    glowColor: 'rgba(239, 68, 68, 0.25)',
    ariaLabel: 'Status: Circuit Open',
  },
  PAUSED: {
    label: 'Paused',
    badgeBg: '#6B72801A',
    badgeText: '#4B5563',
    borderColor: '#6B7280',
    dotBg: '#6B7280',
    glowColor: 'rgba(107, 114, 128, 0.2)',
    ariaLabel: 'Status: Paused',
  },
  ACTIVE: {
    label: 'Active',
    badgeBg: '#3B82F61A',
    badgeText: '#2563EB',
    borderColor: '#3B82F6',
    dotBg: '#3B82F6',
    glowColor: 'rgba(59, 130, 246, 0.25)',
    ariaLabel: 'Status: Active',
  },
  RUNNING: {
    label: 'Healthy',
    badgeBg: '#10B9811A',
    badgeText: '#059669',
    borderColor: '#10B981',
    dotBg: '#10B981',
    glowColor: 'rgba(16, 185, 129, 0.25)',
    ariaLabel: 'Status: Running',
  },
  REMEDIATING: {
    label: 'Remediating',
    badgeBg: '#F59E0B1A',
    badgeText: '#D97706',
    borderColor: '#F59E0B',
    dotBg: '#F59E0B',
    glowColor: 'rgba(245, 158, 11, 0.25)',
    ariaLabel: 'Status: Remediating',
  },
  QUARANTINING: {
    label: 'Quarantining',
    badgeBg: '#F59E0B1A',
    badgeText: '#D97706',
    borderColor: '#F59E0B',
    dotBg: '#F59E0B',
    glowColor: 'rgba(245, 158, 11, 0.25)',
    ariaLabel: 'Status: Quarantining',
  },
  REFETCHING: {
    label: 'Refetching',
    badgeBg: '#06B6D41A',
    badgeText: '#0891B2',
    borderColor: '#06B6D4',
    dotBg: '#06B6D4',
    glowColor: 'rgba(6, 182, 212, 0.25)',
    ariaLabel: 'Status: Refetching',
  },
  REPROCESSING: {
    label: 'Reprocessing',
    badgeBg: '#8B5CF61A',
    badgeText: '#7C3AED',
    borderColor: '#8B5CF6',
    dotBg: '#8B5CF6',
    glowColor: 'rgba(139, 92, 246, 0.25)',
    ariaLabel: 'Status: Reprocessing',
  },
  VALIDATING: {
    label: 'Validating',
    badgeBg: '#3B82F61A',
    badgeText: '#2563EB',
    borderColor: '#3B82F6',
    dotBg: '#3B82F6',
    glowColor: 'rgba(59, 130, 246, 0.25)',
    ariaLabel: 'Status: Validating',
  },
  RESUMING: {
    label: 'Resuming',
    badgeBg: '#10B9811A',
    badgeText: '#059669',
    borderColor: '#10B981',
    dotBg: '#10B981',
    glowColor: 'rgba(16, 185, 129, 0.25)',
    ariaLabel: 'Status: Resuming',
  },
  RECOVERED: {
    label: 'Recovered',
    badgeBg: '#10B9811A',
    badgeText: '#059669',
    borderColor: '#10B981',
    dotBg: '#10B981',
    glowColor: 'rgba(16, 185, 129, 0.25)',
    ariaLabel: 'Status: Recovered',
  },
  RECOVERY_FAILED: {
    label: 'Recovery Failed',
    badgeBg: '#EF44441A',
    badgeText: '#DC2626',
    borderColor: '#EF4444',
    dotBg: '#EF4444',
    glowColor: 'rgba(239, 68, 68, 0.25)',
    ariaLabel: 'Status: Recovery Failed',
  },
  IDLE: {
    label: 'Idle',
    badgeBg: '#6B72801A',
    badgeText: '#6B7280',
    borderColor: '#4B5563',
    dotBg: '#6B7280',
    glowColor: 'transparent',
    ariaLabel: 'Status: Idle',
  },
  UNKNOWN: {
    label: 'Unknown',
    badgeBg: '#6B72801A',
    badgeText: '#9CA3AF',
    borderColor: '#4B5563',
    dotBg: '#9CA3AF',
    glowColor: 'transparent',
    ariaLabel: 'Status: Unknown',
  },
};

export function getStatusStyle(status?: string): StatusStyle {
  const upper = (status || 'UNKNOWN').toUpperCase();
  return STATUS_STYLES[upper] || STATUS_STYLES.UNKNOWN;
}
